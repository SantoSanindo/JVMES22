...
Public Class ProductionRequest
                        ...
                        Else
                            Try
                                Dim sqlInsertInputStockDetail As String = "INSERT INTO stock_card (mts_no,[MATERIAL], [QTY], [INV_CTRL_DATE], [TRACEABILITY], [LOT_NO], [BATCH_NO], [PO], [SUB_SUB_PO], [Finish_Goods_PN], [ACTUAL_QTY],[LINE],[SUB_PO],[STATUS],[DEPARTMENT],[STANDARD_PACK],[QRCODE],[SUM_QTY],[LEVEL],[ID_LEVEL],insert_who,qrcode_new)
                                                VALUES ((select mts_no from stock_card where status = 'Receive From Main Store' and qrcode_new='" & TextBox1.Text & "'),'" & globVar.QRCode_PN & "'," & dtCheckStockMinistore.Rows(0).Item("qty") & ",'" & globVar.QRCode_Inv & "','" & globVar.QRCode_Traceability & "','" & globVar.QRCode_lot & "','" & globVar.QRCode_Batch & "','" & PO.Text & "','" & SubSubPO.Text & "','" & DataGridView3.Rows(CurrentRowIndex).Cells("FG Part Number").Value & "'," & dtCheckStockMinistore.Rows(0).Item("qty") & ",'" & ComboBox1.Text & "','" & SubPO.Text & "','Production Request','" & globVar.department & "',(select standard_pack from stock_card where status = 'Receive From Main Store' and qrcode_new='" & TextBox1.Text & "'),(select qrcode from stock_card where status = 'Receive From Main Store' and qrcode_new='" & TextBox1.Text & "')," & dtCheckStockMinistore.Rows(0).Item("qty") & ",'Fresh','" & globVar.QRCode_PN & "','" & globVar.username & "','" & TextBox1.Text & "')"
                                Dim cmdInsertInputStockDetail = New SqlCommand(sqlInsertInputStockDetail, Database.koneksi)
                                If cmdInsertInputStockDetail.ExecuteNonQuery() Then
    ...
    Sub tampilDataComboBoxLine()
        Call Database.koneksi_database()
        Dim dtMasterLine As DataTable = Database.GetData("select line as name from SUB_SUB_PO ssp, main_po mp where ssp.status='Open' and mp.id=ssp.main_po and mp.department ='" & globVar.department & "' order by ssp.line")
